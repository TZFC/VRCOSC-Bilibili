...
程序支持 12 种相对相机移动指令（6 自由度），通过哔哩哔哩直播弹幕关键词触发：
- **移动与旋转**: 左右移动、上下移动、前后移动、上下倾斜（Pitch）、左右偏转（Yaw）、左右旋转（Roll）。
- **视口相对**: 所有移动和旋转均基于当前相机视口方向（使用 Unity 的左手坐标系 ZXY 欧拉旋转计算）。
- **尊重手动操作**: 当您在游戏内手动调整相机位置或角度时，程序会立即更新本地缓存坐标，并在最新位置的基础上执行后续的相对移动。
- **自定义关键词和步长**: 您可以在 UI 面板中的 **Camera Steps** 和 **Camera Keywords** 栏自定义每个指令的移动步长以及弹幕匹配关键词（支持使用逗号分隔配置多个触发词）。
- **平滑移动**: 支持 VRChat 相机内置的平滑移动模式，无需在外部程序内进行插值计算。
